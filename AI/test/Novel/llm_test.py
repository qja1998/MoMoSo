...
# DeepSeek-R1 모델 로드 (trust_remote_code=True 추가)
model_name = "deepseek-ai/DeepSeek-R1"
tokenizer = AutoTokenizer.from_pretrained(model_name)

# 모델을 로드할 때 quantization_config를 설정하지 않음
model = AutoModelForCausalLM.from_pretrained(
    model_name, 
    trust_remote_code=True,
    torch_dtype=torch.float32,  # FP8 양자화 문제를 피하기 위해 FP32로 강제
    device_map="auto"  # 가능한 디바이스에 자동으로 모델을 배치
)
...
# 프롬프트 템플릿 생성
prompt = prompt_template_1st.format(genre=genre, title=title, worldview=worldview, synopsis=synopsis, characters=characters)

# 모델에 프롬프트 전달하여 생성된 소설 첫 챕터(서문) 받기
