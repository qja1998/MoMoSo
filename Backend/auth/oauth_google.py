...
async def google_callback(response: Response, db: Session = Depends(get_db), code: str = Form(...)):
    """
    ✅ Google OAuth2 인증 후 Access Token을 가져오고, 사용자 정보를 조회한 뒤 JWT 발급
    """
    if not code:
    ...
    if token_response.status_code != 200:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to fetch Google OAuth2 token")
    ...
    if existing_user:
        oauth_connected = False  # OAuth 계정 연결 여부
        ...
        if not existing_user.is_oauth_user:
            ...
            new_oauth = OAuthAccount(
                ...
                account_id=existing_user.email
            )
            db.add(new_oauth)
            db.commit()
    ...
    jwt_access_token = create_access_token(data={"sub": existing_user.email}, expires_delta=access_token_expires)

    refresh_token_expires = timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS)
    jwt_refresh_token = create_refresh_token(data={"sub": existing_user.email}, expires_delta=refresh_token_expires)

    # ✅ 5. Redis에 Refresh Token 저장
    from utils.auth_utils import redis_client  # Redis 유틸 가져오기
    redis_client.setex(f"refresh_token:{existing_user.email}", int(refresh_token_expires.total_seconds()), jwt_refresh_token)
