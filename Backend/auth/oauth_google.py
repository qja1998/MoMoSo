...
from fastapi.responses import RedirectResponse # Import 추가
...
FRONTEND_REDIRECT_URI = os.getenv("FRONTEND_REDIRECT_URI", "http://localhost:5173/auth/login-success") # 프론트엔드 URI
...
async def google_callback(request: Request, response: Response, db: Session = Depends(get_db), redis_client: Redis = Depends(get_redis)):
    """
    Google OAuth2 콜백 처리
    """
    try:
        ...
        try:
            ...
            logger.error(f"Google token 요청 실패: {e}")
        ...
        logger.exception("Google 로그인 콜백 처리 중 오류 발생") # 모든 Exception 로깅
