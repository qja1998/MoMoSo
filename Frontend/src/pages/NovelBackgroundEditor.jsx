...
const NovelBackgroundEditor = () => {
  ...
  const handleWorldviewGenerate = async () => {
    try {
      const response = await axios.post(
        'http://127.0.0.1:8000/api/v1/ai/worldview',
        {
          genre: selectedGenre,
          title: title,
        },
  ...
  const handleSynopsisGenerate = async () => {
    try {
      const response = await axios.post(
        'http://127.0.0.1:8000/api/v1/ai/synopsis',
        {
          genre: selectedGenre,
          title: title,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axios.post(`http://127.0.0.1:8000/api/v1/novel?user_pk=${userPk}`, dataToSend, {
      ...
      if (response.status === 200) {
        ...
        console.error('Error saving novel:', response.status, response.data)
      }
    } catch (error) {
  ...
  const handleAICharacterGenerate = async () => {
    ...
    try {
      const hasCharacterData = characters.some(
        (char) => char.name || char.gender || char.age || char.job || char.profile
      )
        ...
        <Stack direction="column" spacing={2} sx={{ width: '100%', p: 3 }}>
          ...
          <Stack direction="column" spacing={1} sx={{ mb: 2 }}>
            <Stack direction="row" spacing={1} sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <PrimaryButton startIcon={<OfflineBoltIcon />} onClick={handleWorldviewGenerate} sx={{ py: 0.5 }}>
                AI 생성
              </PrimaryButton>
            </Stack>
            <TextField
          ...
          <Stack direction="column" spacing={1} sx={{ mb: 2 }}>
            <Stack direction="row" spacing={1} sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <PrimaryButton startIcon={<OfflineBoltIcon />} onClick={handleSynopsisGenerate} sx={{ py: 0.5 }}>
                AI 생성
              </PrimaryButton>
            </Stack>
            <TextField
