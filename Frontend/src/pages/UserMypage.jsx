...
const UserMypage = () => {
  ...
  const NovelGrid = ({ novels = [] }) => {
    ...
    return (
      <Grid container spacing={2}>
        {novels.map((novel) => (
          <Grid item xs={12} sm={4} md={2} key={novel.novel_pk}>
            <Card
              sx={{
                height: '100%',
                display: 'flex',
            ...
            >
              <CardActionArea onClick={() => handleNovelClick(novel.novel_pk)}>
                ...
                <CardContent sx={{ p: 1.5 }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    최근 작성일: {novel.created_date 
                      ? new Date(novel.created_date).toLocaleDateString('ko-KR')
                      : '날짜 정보 없음'}
                  </Typography>
                </CardContent>
              </CardActionArea>
    ...
    return (
      <Grid container spacing={3}>
        {comments.map((comment) => (
          <Grid item xs={12} sm={6} md={4} key={comment.comment_pk}>
            <Paper
              variant="outlined"
              sx={{
                p: 2,
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Box sx={{ mt: 3 }}>
        {selectedTab === 0 && (
          <NovelGrid novels={profileData.recent_novels} />
        )}
        {selectedTab === 1 && (
          <NovelGrid novels={profileData.liked_novels} />
        )}
        {selectedTab === 2 && (
          <NovelGrid novels={profileData.novels_written} />
        )}
        {selectedTab === 3 && (
          <CommentsSection 
            comments={profileData.comments} 
            cocomments={profileData.cocomments}
          />
        )}
      </Box>
    </Container>
