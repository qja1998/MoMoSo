  ...
  return (
    <Container maxWidth="lg">
      <Box sx={{ py: 4 }}>
        ...
        <Box sx={{ mt: 4 }}>
          <Typography variant="h5" sx={{ mb: 3 }}>최근 본 작품</Typography>
          {mainData?.user?.recent_novels?.length > 0 ? (
            <Grid container spacing={3}>
              {mainData.user.recent_novels.map((novel) => (
                <Grid item xs={12} sm={6} md={3} key={novel.novel_pk}>
                  <Card
                    sx={{
                      height: '100%',
                      display: 'flex',
...
const NonLoggedInContent = React.memo(({ 
  ref1, 
  ref2, 
  ref3, 
  inView1, 
...
const MainPage = () => {
  ...
  const handleNovelClick = useCallback((novelPk) => {
